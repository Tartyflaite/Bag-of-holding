class Knapsack:
    ...
    # Vérifie si le sac peut ajouter l'objet à l'index indiqué et l'ajoute si possible. Return true si l'objet a été
    # ajouté.
    def add_item(self, objects_dict: dict, objects_list: list, index: int) -> bool:
...
# Création d'un arbre décisionnel. Pour chaque item de la liste, deux branches sont créées : l'une où l'item est
# ajouté, l'autre sans ajout. Lorsque la fin de la liste est atteinte, la valeur du knapsack à chaque branche est
# comparé à sa branche voisine et la meilleure est retournée.
        ...
        for j in range(knapsack.capacity + 1):
            if i == 0 or j == 0:
                ...
                if included.get_value_and_weight(objects_dict)[0] > not_included.get_value_and_weight(objects_dict)[0]:
                    ...
                    table[i][j] = not_included.duplicate()
            else:
